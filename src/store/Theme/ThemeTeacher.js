...
const saveThemeToStorage = async theme => {
  try {
    await AsyncStorage.setItem('themeTeacher', JSON.stringify(theme));
  } catch (error) {
    console.error('Error saving theme to AsyncStorage:', error);
  }
};

export const loadTheme = createAsyncThunk('theme/loadThemeteacher', async () => {
  try {
    const theme = await AsyncStorage.getItem('themeTeacher');
    console.log("themteacher", theme);
